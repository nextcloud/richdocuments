OC.L10N.register(
    ...
    "This application can connect to a Office Online (or other) server (WOPI-like Client). Nextcloud is the WOPI Host. Please read the documentation to learn more about that." : "此应用可连接到一台在线（或其他）服务器（类WOPI客户端）。NextCloud则是WOPI主机。更详细信息请阅读文档。",
    ...
    "Enable if your Office Online server uses a self signed certificate" : "若您的Office Online服务器使用自签名证书，请启用",
    ...
    "All users can edit documents with Office Online by default. When this setting is active, only the members of the specified groups can edit and the others can only view documents." : "默认情况下所有用户都能使用在线协作编辑文档。当此设置激活时，只有特定分组的成员才能编辑文档，而其他用户只能查看文档。",
    ...
    "Canonical webroot, in case there are multiple, for Office to use. Provide the one with least restrictions. Eg: Use non-shibbolized webroot if this instance is accessed by both shibbolized and non-shibbolized webroots. You can ignore this setting if only one webroot is used to access this instance." : "给Office使用的canonical webroot（如果有多个）。请提供一个限制最少的。例如，若这个实例同时被shibbolized和非shibbolized的webroot访问，则应选择非shibbolized的webroot。如果只有一个webroot用来访问这个实例，您可忽略此设置。",
